...
const tick_t frequencies[3] = {1000, 200, 100};

/**
 * @brief turns on the onboard LED of the STM 32 and enters a infinite loop
 */
void trigger_error_signal() {
 ...
 *
 * @note in case of a null pointer, trigger_error_signal is called
 */
void delayInit( delay_t * delay, tick_t duration ) {
	if (delay == NULL) {
	    trigger_error_signal();
		return;
	}
 ...
 *  - In case of a null pointer, trigger_error_signal is called
 */
...
int main(void)
{
  ...
  // Init the delay with the first frequency of the frequencies array
  delayInit(&delay, frequencies[freq_idx]);
  uint32_t amount_of_frequencies = sizeof(frequencies) / sizeof(uint32_t);
  ...
  while (1)
  {
    ...
    /* USER CODE BEGIN 3 */
	  ...
	  if (blink_counter == AMOUNT_OF_BLINKS) {
	  	freq_idx = (freq_idx + 1) % amount_of_frequencies;
	  	blink_counter = 0;
	  	delayWrite(&delay, frequencies[freq_idx]);
	  }
  }
  /* USER CODE END 3 */
}
